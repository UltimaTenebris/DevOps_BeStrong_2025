terraform {
  required_providers {
    azurerm = {
      ...
      # FIX: Bump version to ensure 'storage_account' block is supported
      version = "~> 3.100" 
    }
  }
  # Note: Ensure your backend block is correct here if you are using one
  #backend "azurerm" {
  #  resource_group_name  = "rg-bestrong-state"
  #  storage_account_name = "stbestrongstate"
...
resource "azurerm_linux_web_app" "app" {
  ...
  # FIX: storage_account is a SIBLING to site_config (at root of resource)
  # This requires azurerm provider version >= 3.60
  storage_account {
